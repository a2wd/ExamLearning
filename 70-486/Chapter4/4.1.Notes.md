...
## Part 4 - Debugging on Azure

Windows Azure has lots of logging features but this data must be stored elsewhere than the application logs as they are destroyed on application stop.

Diagnostics can be added via the `ServiceDefinition.csdef` file:
...
and must also be included in `ServiceConfiguration.cscfg` (either the ServiceConfiguration.Cloud.cscfg or ServiceConfiguration.Local.cscfg).
...
Azure logs, IIS logs and Azure Infrastructure diagnostic logs are captured by default. You can save the logs to a data storage account in Azure.

Extra services can be added with `Diagnostics.wadcfg` which is in the bin folder for Web roles and the root for Worker roles.

These can be added via code, but the configuration files are better because changes don't require a role restart and they will be run before the `OnStart` method is even called.

The following example is a Diagnostics.wadcfg which adds two performance counters:
...
You can transfer data to storage on-demand or on-schedule.

Diagnostics comes with a processing cost and a time cost to sift through data - you should be selective about what you log/trace.

**Debugging Azure** can be done locally with the Azure SDK which includes Azure emulators. Azure also provides IntelliTrace which are logs of the application you can run locally to debug. You can enable IntelliTrace during debug-deployment and can set events-only or events & call information.

You can also enable RDP debugging during the Azure deployment, this allows you to view logs on the server.