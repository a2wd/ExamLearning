...
## Part 4 - Maintain data integrity

In an app, in addition to authentication and authorisation, you can secure data at-rest with encryption. SHA is better than MD5 for hashing, AES is better than DES for for encryption and is symmetric.

SSL uses asymmetric encryption. Symmetric encryption is for streams and good for large amounts of data, but assymetric enryption is better for smaller amounts of data as it works on the whole file.

In `System.Security.Cryptography`, there is `AsymmetricAlgorithm` and `SymmetricAlgorithm`, such as `Rijandael` or `RijandaelManaged`. For streaming ciphers, you also need to use a `CryptoStream`.
...
Alternatively, assymetric ciphers could be used:
...
Where RSAKeyInfo contains the public key.

When dealing with keys you should:

1. Use unique keys
2. Protect the keys
3. Don't store keys with data
4. Configure keys to expire.

**Encrypt the web.config data** - you can use `DPAPIProtectedConfigurationProvider` or `RsaProtectedConfigurationProvider`, the latter is useful for when you need to work on multiple servers as it allows for export of keys.

Use **aspnet_regiis.exe** to encrypt the web.config:

`aspnet_regiis -pe "ConnectionStrings" -app "/MachineDPAPI" -prov "RsaProtectedConfigurationProvider"`

Use `-pd` instead of `-pe` to decrypt the web.config with *aspnet_regiis.exe*.

You can **sign app data** to prevent tampering: encryption is grouped with digital signatures in asymmetric algorithms. One example is DSA - the classes have a static method ie; `DSA.Create()` and then you can use the reurned object to `dsaObject.CreateSignature(byte hashCode)`. You can also verify a signature from a hashCode with `.VerifySignature(byte sig, byte hashCode)`.

The **RSACryptoServiceProvider** and **DSACryptoServiceProvider** both have four methods to sign and hash:
...
## Part 5 - A secure site

**SSL** with certificates is good for securing communications between a user and the site. They are called public key, digital or identity certificates. It links an identity to a public key with a signature.

The certificate can be signed by a CA, an **endorsement** by a third party or as a SSC.

In IIS, you can create a public/private keypair - the public key is sent to a CA and the responding certficate can be loaded into IIS. After this you can set up an https binding in IIS and set `RequireHttps` on controllers, actions or globally in `GlobalFilters`.

**Salting and hashing passwords** is important to avoid giving away user passwords in a hack and to ensure better encryption of data. They help counter rainbow tables and brute force/dictionary cracking.

You shouldn't reuse salts and salt-length should be as long as the hashing output (ie, SHA256 output is 256 bytes/32 bits - use a 32 bit salt).

Salts should be cryptographically secure - derive them with `RNGCryptoServiceProvider.GetBytes`. 

To impede hackers, you can also **key-stretch** which is adding extra information such as a HMAC an minimum of 1000 times to a value being hashed. 

Password reset should be handled with an email-reset loop.

**HTML Encoding** and the **AntiXSS** library can prevent cross-site scripting. You can use `Html.Encode(stringData)` to encode html entities. You can use `Server.HtmlEncode` to encode data in controllers such as when saving to the database. AntiXSS is better because it is faster, takes an accepted-list approach to sanitation and is standards compliant.

**Deferred validation** is possible, such as with a tabbed multi-part form by setting `<httpRuntime requestValidationMode="4.5" .. />` in the *web.config*. This means form data is only validated when accessed and only for parts of the form that are accessed. You can still access unvalidated data from `context.Request.Unvalidated.Form["value"]`.

**Parameterise SQL queries** and don't just accept data:

1. Check data for length, type & formatting
2. Use type-safe SQL params
3. Use a restricted account for DB access
4. Don't ever show error messages to the user

An example of a parameterised request is:
...
Another vector is **CSRF, cross-site request forgery** and can be mitigated with the `@Html.AntiForgeryToken()` and in a controller method with `[ValidateAntiForgeryToken]`. Cookies must be enabled for this to work.